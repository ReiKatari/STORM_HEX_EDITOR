...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                             ...
                             QAbstractScrollArea, QSpinBox, QTextEdit, QProgressBar, QToolTip, QAbstractButton)
...
# --- STORM SUITE IMPORTS ---
# from stormsuite import LauncherDialog # Removed to avoid cycle
...
class HexWidget(QAbstractScrollArea):
    ...
    def contextMenuEvent(self, event):
        ...
        if self.cursor_pos != -1:
             # Just emit signal or handle logic differently?
             # For now, disable Bookmark context menu in Widget standalone
             # Or use signal: self.bookmarkRequested.emit(self.cursor_pos)
             pass
...
class SearchDialog(QDialog):
    def __init__(self, parent):
        super().__init__(parent)
        self.setWindowTitle(LOCALE[parent.current_lang]["find"])
        layout = QHBoxLayout(self)
        self.input_search = QLineEdit()
        self.mode = "hex"  # hex, ascii
        layout.addWidget(QLabel(LOCALE[parent.current_lang]["pattern"] + ":"))
        layout.addWidget(self.input_search)
        btn = QPushButton(LOCALE[parent.current_lang]["find"])
...
class GoToDialog(QDialog):
    def __init__(self, parent):
        super().__init__(parent)
        self.setWindowTitle(LOCALE[parent.current_lang]["goto"])
        layout = QHBoxLayout(self)
        self.input_offset = QLineEdit()
        layout.addWidget(QLabel(LOCALE[parent.current_lang]["offset"] + " (Hex):"))
        layout.addWidget(self.input_offset)
        btn = QPushButton(LOCALE[parent.current_lang]["goto"])
        btn.clicked.connect(self.accept)
        layout.addWidget(btn)
...
class BaseConverterDialog(QDialog):
    def __init__(self, parent):
        super().__init__(parent)
        self.setWindowTitle(LOCALE[parent.current_lang]["base_conv"])
...
class BitwiseDialog(QDialog):
    def __init__(self, parent):
        super().__init__(parent)
        self.setWindowTitle(LOCALE[parent.current_lang]["bitwise"])
        layout = QVBoxLayout(self)
        
        self.combo_op = QComboBox()
        self.combo_op.addItems(["AND", "OR", "XOR", "NOT", "Shift Left", "Shift Right", "Rotate Left", "Rotate Right"])
        layout.addWidget(QLabel(LOCALE[parent.current_lang].get("operation", "Operation") + ":"))
        ...
        btn = QPushButton(LOCALE[parent.current_lang]["apply_patch"] if "apply_patch" in LOCALE[parent.current_lang] else "Apply") # Approximate
        btn.clicked.connect(self.accept)
        layout.addWidget(btn)
...
class SignatureDialog(QDialog):
    def __init__(self, parent, data_source):
        super().__init__(parent)
        self.setWindowTitle(LOCALE[parent.current_lang]["signature"])
        ...
        btn_analyze = QPushButton(LOCALE[parent.current_lang]["analyze"])
